...
public class EthernetUtil {
    ...
    public static boolean setEthernetStaticIp(Context context, String address, String mask, String gate, String dns) {
        try {
            ...
            // 重启心态网，可通过开启usb调试后，再关闭来实现插拔效果
            new Thread(new Runnable() {
                @Override
                public void run() {
                    SystemClock.sleep(2000);
                    SettingsHelper.get().putIntValue(SettingUtils.General.USB_DEBUG_ENABLE, 1);
                    SettingsHelper.get().putIntValue(SettingUtils.General.USB_DEBUG_ENABLE, 0);
                }
            }).start();
            return true;
        } catch (Exception e) {
